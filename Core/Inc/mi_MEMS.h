...
//static volatile uint8_t MagCalRequest = 1;
...
/**
  * @brief  Funcion de inicializacion de megnetometro y acelerometro del
  * paquete de bibliotecas de ST X-CUBE-MEMS1
  * @retval None
  */
 void MX_MEMS_Init(void)
{
...
//	} MFX_output_t;
 ...
 void MotionFX_manager_init(void)
 {
   ...
   ipKnobs->output_type = MFX_ENGINE_OUTPUT_ENU; //apunta Este Norte Arriba
   ipKnobs->LMode = 1;
   ipKnobs->modx = 1;
...
void BSP_SENSOR_ACC_GetOrientation(char *Orientation)
{
  Orientation[0] = 's'; //x del accelerometro & giroscopio
  Orientation[1] = 'e'; //y del accelerometro & giroscopio
...
void BSP_SENSOR_MAG_GetOrientation(char *Orientation)
{
  Orientation[0] = 'n'; //x del magnetometro
  Orientation[1] = 'w'; //y del magnetometro
